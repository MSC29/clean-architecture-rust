...
async fn test_should_return_multiple_results() {
    ...
    let response = reqwest::get(&format!("{}/cats/", &address)).await.expect("Failed to execute request.");
    ...
    assert_eq!(content_json[0].fact, "The first true cats came into existence about 12 million years ago and were the Proailurus.");
...
async fn test_should_return_one_results_only() {
    ...
    let response = reqwest::get(&format!("{}/cats/random", &address))
        .await
