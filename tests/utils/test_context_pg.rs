...
impl TestContextPostgreSQL {
    pub fn new(base_url: &str, db_name: &str) -> Self {
        ...
        let conn_create_db = PgConnection::establish(&postgres_url).expect("Cannot connect to postgres database.");
        ...
        let conn = PgConnection::establish(&format!("{}/{}", base_url, db_name)).unwrap_or_else(|_| panic!("Cannot connect to {} database", db_name));
...
impl Drop for TestContextPostgreSQL {
    fn drop(&mut self) {
        ...
        let conn = PgConnection::establish(&postgres_url).expect("Cannot connect to postgres database.");
